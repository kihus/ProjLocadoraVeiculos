...
(@LocacaoID1, 1, 1, GETDATE(), DATEADD(day, 5, GETDATE()), 80.00, 'Ativa');
...
(3, 5, DATEADD(day, -10, GETDATE()), DATEADD(day, -5, GETDATE()), DATEADD(day, -5, GETDATE()), 180.00, 900.00, '1');
...
CREATE OR ALTER PROCEDURE sp_CancelarLocacao @idLocacao UNIQUEIDENTIFIER,
                                             @Status VARCHAR(20)
AS
BEGIN
    SET NOCOUNT ON;
...
BEGIN
    ...
    BEGIN TRY
        SELECT Nome FROM tblClientes WHERE ClienteID = @idCliente;
    END TRY
    BEGIN CATCH
